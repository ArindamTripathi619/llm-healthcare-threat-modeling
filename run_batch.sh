...
echo "Starting Orchestrator..." | tee -a "$LOG_FILE"
# Start Orchestrator in background
nohup venv/bin/uvicorn experiments.orchestrator.main:app --host 127.0.0.1 --port 8000 > nohup.out 2>&1 &
SERVER_PID=$!
...
venv/bin/python experiments/client/batch_runner.py 2>&1 | tee -a "$LOG_FILE"
